...
void djikstra()
{
	...
	while (!fila.empty())
	{
        ...
        for (int i = -1; i <=1; i++)
        {
            ...
            if (weight < dists[newX][newY][newDir][ newDir == dir ? count+1 : 1])
            {
                dists[newX][newY][newDir][ newDir == dir ? count+1 : 1] = weight;
                fila.push({newX,newY,weight, newDir, newDir == dir ? count+1 : 1});     
            }
        }
	}
    int mini  = iinf;
    for (int x = 0; x < 4; x++)
        for (int l = 0; l < 4; l++)
            mini = min(mini, dists[n-1][n-1][x][l]);
